...
def main():
    ...
    sleep(5)  # Espera 5 segundos para asegurar que todo esté listo
    # Ejecuta el modelo USG
    resultado = subprocess.run('wine USGs_1.exe mpupio_0.nam', shell=True, capture_output=True, text=True)
    ...
    print("resultados zonbudusg_almac:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    resultado = subprocess.run('wine zonbudusg.exe < zonbudusg_almac.in', shell=True, capture_output=True, text=True)
    print("resultados zonbudusg_secc_2024:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    resultado = subprocess.run('wine zonbudusg.exe < zonbudusg_secc_2024.in', shell=True, capture_output=True, text=True) 
    ...
    shutil.copy(os.path.join(cwd, 'model', 'balance_almac.2.csv'), os.path.join(cwd, 'resultados', 'balance_almac_{}.csv'.format(run)))
    shutil.copy(os.path.join(cwd, 'model', 'balance_secc_2024.2.csv'), os.path.join(cwd, 'resultados', 'balance_secc_2024_{}.csv'.format(run)))
