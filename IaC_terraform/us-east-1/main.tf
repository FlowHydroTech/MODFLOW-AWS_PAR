...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRoleAgenteMpupio"
...
resource "aws_ecs_cluster" "cluster-agente-pelambres" {
...
/*resource "aws_cloudwatch_log_group" "ecs_logs_choapa_v2" {
  name              = "/ecs/agente_choapa_v2"
  retention_in_days = 30
  tags              = var.common_tags_icsara
}
resource "aws_cloudwatch_log_group" "ecs_logs_choapa_cb_v2" {
  name              = "/ecs/agente_choapa_cb_v2"
  retention_in_days = 30
  tags              = var.common_tags_icsara
}
resource "aws_cloudwatch_log_group" "ecs_logs_choapa_evu_v2" {
  name              = "/ecs/agente_choapa_evu_v2"
  retention_in_days = 30
  tags              = var.common_tags_icsara
}*/
#reutilización de logs de CloudWatch existentes
data "aws_cloudwatch_log_group" "ecs_logs_choapa_v2" {
  name              = "/ecs/agente_choapa_v2"
  tags              = var.common_tags_icsara
}

data "aws_cloudwatch_log_group" "ecs_logs_choapa_cb_v2" {
  name              = "/ecs/agente_choapa_cb_v2"
  tags              = var.common_tags_icsara
}

data "aws_cloudwatch_log_group" "ecs_logs_choapa_evu_v2" {
  name              = "/ecs/agente_choapa_evu_v2"
  tags              = var.common_tags_icsara
}
...
resource "aws_ecs_task_definition" "task_agente_choapa" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        awslogs-group         = data.aws_cloudwatch_log_group.ecs_logs_choapa_v2.name
        awslogs-region        = var.aws_region
        awslogs-stream-prefix = "ecs"
      }
    }
...
resource "aws_ecs_task_definition" "task_agente_choapa_cb" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        awslogs-group         = data.aws_cloudwatch_log_group.ecs_logs_choapa_cb_v2.name
        awslogs-region        = var.aws_region
        awslogs-stream-prefix = "ecs"
      }
    }
...
resource "aws_ecs_task_definition" "task_agente_choapa_evu" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        awslogs-group         = data.aws_cloudwatch_log_group.ecs_logs_choapa_evu_v2.name
        awslogs-region        = var.aws_region
        awslogs-stream-prefix = "ecs"
      }
    }
